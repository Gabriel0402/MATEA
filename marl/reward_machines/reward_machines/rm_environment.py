...
from reward_machines.ma_reward_machine import MARewardMachine
import copy
...
class MARewardMachineEnv(gym.Wrapper):
    def __init__(self, env, rm_files_list,num_agents):
        """
        Multi-agent RM environment
        --------------------
        It adds a set of RMs to the environment for each agent:
            - Every episode, each agent has to solve a different RM task
            - This code keeps track of the current state of each agent's current RM task
            - The id of the RM state is appended to the observations
            - The reward given to each agent comes from their respective RM

        Parameters
        --------------------
            - env: original environment. It must implement the following function:
                - get_events(...): Returns the propositions that currently hold on the environment.
            - rm_files_list: list of lists of strings with paths to the RM files for each agent.
        """
        super().__init__(env)

        # Loading the reward machines for each agent
        self.rm_files_list = rm_files_list
        self.reward_machines_list = []
        self.num_rm_states_list = []
        for rm_files in rm_files_list:
            reward_machines = []
            num_rm_states = 0
            for rm_file in rm_files:
                rm = MARewardMachine(rm_file)
                num_rm_states += len(rm.get_states())
                reward_machines.append(rm)
            self.num_rm_states_list.append(num_rm_states)
            self.reward_machines_list.append(reward_machines)
        self.num_agents = num_agents

        # The observation space is a dictionary including the env features and a one-hot representation of the state in the reward machines for each agent
        self.observation_dict = {}
        for agent_id in range(self.num_agents):
            observation_dict_agent = spaces.Dict({'features': env.observation_space, 'rm-state': spaces.Box(low=0, high=1, shape=(self.num_rm_states_list[agent_id],), dtype=np.uint8)})
            self.observation_dict[agent_id] = observation_dict_agent
        flatdims = [gym.spaces.flatdim(observation_dict_agent) for observation_dict_agent in self.observation_dict.values()]
        flatspace = spaces.Tuple([spaces.Box(low=s_low, high=s_high, shape=(flatdim,), dtype=np.float32) for (flatdim,s_low,s_high) in zip(flatdims,env.observation_space.low,env.observation_space.high)])
        self.observation_space = flatspace

        # Computing one-hot encodings for the non-terminal RM states for each agent
        self.rm_state_features = {}
        for agent_id, reward_machines in enumerate(self.reward_machines_list):
            for rm_id, rm in enumerate(reward_machines):
                i=0
                for u_id in rm.get_states():
                    u_features = np.zeros(self.num_rm_states_list[agent_id])
                    u_features[i] = 1
                    i+=1
                    self.rm_state_features[(agent_id, rm_id, u_id)] = u_features
        self.rm_done_feat = np.zeros(max(self.num_rm_states_list)) # for terminal RM states, we give as features an array of zeros

        # Selecting the current RM task for each agent
        self.current_rm_ids = [-1] * self.num_agents
        self.current_rms = [None] * self.num_agents
        self.current_u_ids = [None] * self.num_agents

    def reset(self):
        # Reseting the environment and selecting the next RM tasks for each agent
        self.obs = self.env.reset()
        for agent_id in range(self.num_agents):
            self.current_rm_ids[agent_id] = (self.current_rm_ids[agent_id] + 1) % len(self.rm_files_list[agent_id])
            self.current_rms[agent_id] = self.reward_machines_list[agent_id][self.current_rm_ids[agent_id]]
            self.current_u_ids[agent_id] = self.current_rms[agent_id].reset()

        # Adding the RM states to the observations
        return self.get_observation(self.obs, self.current_rm_ids, self.current_u_ids, [False] * self.num_agents)

    def step(self, actions):
        # executing the actions in the environment
        next_obs, original_rewards, env_dones, infos = self.env.step(actions)
     
        # getting the output of the detectors and saving information for generating counterfactual experiences
        true_props_list = self.env.get_events()
        # print("182",true_props_list)
        self.crm_params_list = {}
        for agent_id in range(self.num_agents):
            true_props = true_props_list[agent_id]
            self.crm_params_list[agent_id]=(self.obs[agent_id], actions[agent_id], next_obs[agent_id], env_dones[agent_id], true_props, infos)
            self.obs[agent_id] = next_obs[agent_id]

        # update the RM states for each agent
        rm_rews = [0] * self.num_agents
        rm_dones = [False] * self.num_agents
        for agent_id in range(self.num_agents):
            self.current_u_ids[agent_id], rm_rew, rm_done = self.current_rms[agent_id].step(self.current_u_ids[agent_id], true_props_list, infos)
            rm_rews[agent_id] = rm_rew
            rm_dones[agent_id] = rm_done

        # returning the results of the actions for each agent
        dones = [rm_done or env_done for rm_done, env_done in zip(rm_dones, env_dones)]
        rm_obs = self.get_observation(next_obs, self.current_rm_ids, self.current_u_ids, dones)
        infos["rewards"]=rm_rews
        infos["dones"] = dones
        return rm_obs, sum(rm_rews), all(dones), infos

    def get_observation(self, next_obs, rm_ids, u_ids, dones):
        rm_obs = []
        for agent_id, rm_id, u_id, done in zip(range(self.num_agents), rm_ids, u_ids, dones):
            rm_feat = self.rm_done_feat if done else self.rm_state_features[(agent_id, rm_id, u_id)]
            agent_obs = {'features': next_obs[agent_id], 'rm-state': rm_feat}
            rm_obs.append(agent_obs)
        rm_obs = tuple(rm_obs)
        return [gym.spaces.flatten(self.observation_dict[i], rm_obs[i]) for i in range(self.num_agents)]
    
    def get_single_observation(self, next_obs, rm_id, u_id, done,agent_id):
        rm_feat = self.rm_done_feat if done else self.rm_state_features[(agent_id,rm_id,u_id)]
        rm_obs = {'features': next_obs,'rm-state': rm_feat}
        return gym.spaces.flatten(self.observation_dict[agent_id], rm_obs)       

class RewardMachineWrapper(gym.Wrapper):
    def __init__(self, env, add_crm, add_rs, add_dcg, gamma, rs_gamma):
        """
        RM wrapper
        --------------------
        ...
        if add_rs:
            for rm in env.reward_machines:
                rm.add_reward_shaping(gamma, rs_gamma,True)
        if add_dcg:
            for rm in env.reward_machines:
                rm.add_reward_shaping(gamma, rs_gamma,False)
...
class MARewardMachineWrapper(gym.Wrapper):
    def __init__(self, env, add_crm, add_rs, add_dcg, gamma, rs_gamma):
        """
        RM wrapper
        --------------------
        ...
        if add_rs:
            for rm in env.reward_machines_list:
                rm[0].add_reward_shaping(gamma, rs_gamma,True)
        if add_dcg:
            for rm in env.reward_machines_list:
                rm[0].add_reward_shaping(gamma, rs_gamma,False)
    ...
    def reset(self):
        # self.valid_states = None # We use this set to compute RM states that are reachable by the last experience (None means that all of them are reachable!) 
        self.valid_states = [None]*self.num_agents
        self.current_rm_id = [0]*self.num_agents
        return self.env.reset()

    def step(self, actions):
        # RM and RM state before executing the action
        all_info = [{}]*self.num_agents
        all_done = [False]*self.num_agents
        all_rm_obs = [None]*self.num_agents
        all_rm_rew = [None]*self.num_agents
        
        rm_obs, rm_rew, done, info = self.env.step(actions)
        info["crm-experience"]={}
        info["rs-reward"]={}
        
        for agent_id in range(self.env.num_agents):
            rm_id = self.env.current_rm_ids[agent_id]
            rm    = self.env.current_rms[agent_id]
            u_id  = self.env.current_u_ids[agent_id]
            if self.add_crm:
                crm_experience = self._get_crm_experience(*self.env.crm_params_list[agent_id],agent_id)
                info["crm-experience"][agent_id] = crm_experience
            elif self.add_rs:
                # Computing reward using reward shaping
                _, _, _, rs_env_done, rs_true_props, rs_info = self.env.crm_params_list[agent_id]
                _, rs_rm_rew, _ = rm.step(u_id, rs_true_props, rs_info, self.add_rs, rs_env_done)
                info["rs-reward"][agent_id] = rs_rm_rew
        return rm_obs, rm_rew, done, info

    def _get_rm_experience(self, rm_id, rm, u_id, obs, action, next_obs, env_done, true_props, info,agent_id):
        rm_obs = self.env.get_single_observation(obs, rm_id, u_id, False,agent_id)
        next_u_id, rm_rew, rm_done = rm.step(u_id, true_props, info, self.add_rs, env_done)
        done = rm_done or env_done
        rm_next_obs = self.env.get_single_observation(next_obs, rm_id, next_u_id, done,agent_id)
        return (rm_obs,action,rm_rew,rm_next_obs,done), next_u_id

    def _get_crm_experience(self, obs, action, next_obs, env_done, true_props, info,agent_id):
        """
        Returns a list of counterfactual experiences generated per each RM state.
        Format: [..., (obs, action, r, new_obs, done), ...]
        """
        ...
        for agent_id, reward_machines in enumerate(self.reward_machines_list):
            ...
            for rm_id, rm in enumerate(reward_machines):
                for u_id in rm.get_states():
                    exp, next_u = self._get_rm_experience(rm_id, rm, u_id, obs, action, next_obs, env_done, true_props, info,agent_id)
                    reachable_states.add((rm_id,next_u))
                    if self.valid_states[agent_id] is None or (rm_id,u_id) in self.valid_states[agent_id]:
                        # We only add experience that are possible (i.e., it is possible to reach state u_id given the previous experience)
                        experiences.append(exp)
...
class HierarchicalRMWrapper(gym.Wrapper):
    ...
    def __init__(self, env, r_min, r_max, use_self_loops, add_rs, add_dcg, gamma, rs_gamma):
        self.r_min = r_min
        self.r_max = r_max
        super().__init__(env)
