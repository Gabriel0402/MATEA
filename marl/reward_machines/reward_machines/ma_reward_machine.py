...
from reward_machines.reward_machine_utils import evaluate_dnf, value_iteration,value_dcg,join_strings_with_ampersand
import time
from baselines import logger

class MARewardMachine:
    ...
    def add_reward_shaping(self, gamma, rs_gamma, flag):
        """
        ...
        if flag:
            ...
            self.potentials = value_dcg(self.U, self.delta_u, self.delta_r, self.terminal_u, rs_gamma)
    ...
    def _compute_next_state(self, u1, true_props):
        props = join_strings_with_ampersand(true_props)
        for u2 in self.delta_u[u1]:
            logger.record_tabular("dnf", self.delta_u[u1][u2])
            logger.record_tabular("true_props", props)
            # print("DEBUG: evaluate dnf")
            # print(self.delta_u[u1][u2],true_props)
            # print(evaluate_dnf(self.delta_u[u1][u2], true_props))
            if evaluate_dnf(self.delta_u[u1][u2], props):
                return u2
        return u1 # no transition is defined for true_props

    def get_next_state(self, u1, true_props):
        props = tuple(true_props)
        if (u1,props) not in self.known_transitions:            
            u2 = self._compute_next_state(u1, true_props)
            self.known_transitions[(u1,props)] = u2
        # print(self.known_transitions[(u1,true_props)])
        return self.known_transitions[(u1,props)]
