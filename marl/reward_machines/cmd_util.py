...
from reward_machines.rm_environment import RewardMachineWrapper,MARewardMachineWrapper, HierarchicalRMWrapper
...
def make_env(env_id, env_type, args, mpi_rank=0, subrank=0, seed=None, reward_scale=1.0, gamestate=None, flatten_dict_observations=True, wrapper_kwargs=None, env_kwargs=None, logger_dir=None, initializer=None):
    ...
    if args.alg.endswith("hrm") or args.alg.endswith("dhrm"):
        env = HierarchicalRMWrapper(env, args.r_min, args.r_max, args.use_self_loops, args.use_rs,args.use_dcg,args.gamma, args.rs_gamma)

    if args.use_rs or args.use_crm or args.use_dcg:
    # if args.use_rs or args.use_crm:
        if args.ma:
            env = MARewardMachineWrapper(env, args.use_crm, args.use_rs, args.use_dcg, args.gamma, args.rs_gamma)
        else:
            env = RewardMachineWrapper(env, args.use_crm, args.use_rs, args.use_dcg, args.gamma, args.rs_gamma)
...
def common_arg_parser():
    ...
    parser.add_argument("--use_dcg", help="Use DCG", action="store_true", default=False)
    ...
    parser.add_argument("--ma", help="Use multi-agent environment and algorithm", action="store_true", default=False)
    parser.add_argument("--dcent", help="Use decentralized algorithm", action="store_true", default=False)
    parser.add_argument('--num_agent', help='number of agents', type=int, default=3)
    return parser
